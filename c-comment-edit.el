...
;;	Date: 1989-01-12 17:36:19 GMT Kyle Jones
;;
...
;;; Change Log:
;;
;; Jun	30	2013	[jari]		24.3	v1.19		Released
;; - Checkdoc and code clean up. Add defcustom. Modernize to current Emacs.
;; - Correct byte compiler errors.
;;
...
;;   .el with the "c-comment-edit.el" -- I do plan to update this if it has
...
(defgroup c-comment-edit nil
  "C-style multi line comment editing mode."
  :group 'tools)

(defvar c-comment-edit-mode-map nil
  "C comment edit map.")

(unless c-comment-edit-mode-map
  (setq c-comment-edit-mode-map (make-sparse-keymap))
  ;; keys;
  (if (string< emacs-version "19")
      (progn
        (define-key c-comment-edit-mode-map "\C-c\C-c" 'c-comment-edit-end)
        (define-key c-comment-edit-mode-map "\e" 'c-comment-edit-abort))
    (define-key c-comment-edit-mode-map "\C-c\C-c" 'c-comment-edit-end)
    (define-key c-comment-edit-mode-map "\e\e" 'c-comment-edit-abort)))

(defcustom c-comment-edit-hook nil
...
(defcustom c-comment-edit-exit-hook nil
  "*Hook to run at the end of edit.
...
(defcustom c-comment-edit-end-hook nil
  "*Hook to run after comment prefix lines have been drawn.
When user has pressed \\[c-comment-edit-end] or ESC to complete editing, the
...
(defcustom c-comment-edit-load-hook nil
...
(defconst c-comment-edit-beg-c nil
  "Start of comment.
After the comment is edited, this variable contains begin MARK of comment.")

(defconst c-comment-edit-end-c nil
  "End of commment.
After the comment is edited, this variable contains end MARK of comment.")

(defconst c-comment-edit-code-buffer nil
  "Buffer name of code.
...
(defcustom c-comment-edit-mode 'text-mode
...
(defcustom c-comment-edit-bname " *C Comment Edit*"
  "*Buffer name to edit the comment."
  :type  'buffer
  :group 'c-comment-edit)

(defcustom c-comment-edit-empty-1-line nil
...
(defcustom c-comment-edit-other-buffer t
...
(defmacro c-comment-save-point (&rest body)
  "Save value of point, evalutes BODY and restore value of point.
...
(defun c-comment-area (beg end)
  "Search area bounds delimited by strings BEG and END.
...
(defun c-comment-find-buffer (&optional buffer)
 "Return data for BUFFER from `c-comment-edit-buffer-alist'."
  ...
  (let ((tuple (c-comment-find-buffer))
    ...
    (let ((inhibit-quit		t)
      ...
      (cond
       ((buffer-modified-p)
        ...
        ;; indent if necessary
        (let ((indention
               (with-current-buffer c-comment-c-buffer
	...
	(setq char-count (- (point-max) (point-min)) )
        (with-current-buffer c-comment-c-buffer
          ...
          (goto-char c-comment-start)
          (insert-buffer-substring c-comment-buffer)
          ...
          (setq c-comment-edit-beg-c (marker-position c-comment-start)
                ...
                c-comment-edit-code-buffer  c-comment-c-buffer)
      ...
      (with-current-buffer c-comment-buffer
...
Look for SEARCH-PREFIX in range COM-START and COM-END.
...
original, use \\[c-comment-edit-abort]."
  ...
  (catch 'out
    (let ((inhibit-quit	t)
      ...
      (set-buffer-modified-p nil))
    (use-local-map  c-comment-edit-mode-map)
...
;; - Here is one example block for C/C++ functions
;;
;; /*************************************************************************
...
;;   (local-set-key "\M-c" 'c-comment-edit)) ;Don't use M-c myself in C/C++
;;
